...
mixin DirectedValueGraphMixin on DirectedGraphMixin
  ...
  void setToBy<T>(from, to, val) => setTo(from, to, T, val);
  ...
  bool hasEdgeToBy<T>(from, to) => hasEdgeTo(from, to, T);
  ...
  bool unSetToBy<T>(from, to) => unSetTo(from, to, T);
  ...
  Iterable valueTosBy<T>(val) => valueTos(val, T);

  Iterable valueFromsBy<T>(val) => valueFroms(val, T);
}
